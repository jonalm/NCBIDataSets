...
# Identifier resolution and flag lowering are handled by `_DatasetsCommand`
# (command.jl); each `download_*` below just names its subcommand and id-kinds.
#
...
# Specialize the "download" verb: compute the output path (it both goes in as
# --filename and comes back as the DataPackage), build the command, invoke it,
# and hand back a handle.
function _download(subcmd::Vector{String}, idkinds::Tuple;
                   filename::Union{Nothing,AbstractString} = nothing,
                   ambiguous = (), api_key = nothing, kw...)
    out = filename === nothing ? _tempzip() : String(filename)
    cmd = _DatasetsCommand("download", subcmd, idkinds;
                           fixed = String["--filename", out], ambiguous, kw...)
...
function download_virus_protein(name::AbstractString;
    ...
    out = filename === nothing ? _tempzip() : String(filename)
    cmd = _DatasetsCommand("download", ["virus", "protein"], _Positional(String[String(name)]);
        fixed = String["--filename", out],
        host, lineage, geo_location, complete_only, refseq, annotated,
        include, released_after, updated_after, kwargs...)
    _invoke(cmd; progress = true, api_key)
    return DataPackage(out)
end
